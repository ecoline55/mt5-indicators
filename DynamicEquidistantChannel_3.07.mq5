...
//--- Вид основного канала во время активной динамики
enum ENUM_ACTIVE_CHANNEL_VIEW
  {
   CHANNEL_VIEW_AS_IS = 0, // Обычный вид
   CHANNEL_VIEW_PALE  = 1, // Бледный
   CHANNEL_VIEW_HIDE  = 2  // Скрыть
  };
input ENUM_ACTIVE_CHANNEL_VIEW InpActiveChannelView = CHANNEL_VIEW_PALE; // Вид активного канала
input double InpActiveChannelPaleMix = 0.75; // Доля фона для бледного вида (0.10..0.90)
...
//+------------------------------------------------------------------+
//| Вид основного канала: исходный цвет сохраняем в GV терминала.    |
//| Следы всегда используют сохраненный исходный цвет.               |
//+------------------------------------------------------------------+
#define DEQ_VIEW_GV_PREFIX "DEQ_VIEW_"
#define DEQ_VIEW_TF_GV_PREFIX "DEQ_VIEW_TF_"

string ChannelViewGV(const string chan)
  {
   return DEQ_VIEW_GV_PREFIX+chan;
  }

string ChannelViewTFGV(const string chan)
  {
   return DEQ_VIEW_TF_GV_PREFIX+chan;
  }

color BlendColor(const color base,const color back,const double mix)
  {
   int br=(int)(base & 0xFF), bg=(int)((base>>8) & 0xFF), bb=(int)((base>>16) & 0xFF);
   int rr=(int)(back & 0xFF), rg=(int)((back>>8) & 0xFF), rb=(int)((back>>16) & 0xFF);
   int r=(int)MathRound(br+(rr-br)*mix);
   int g=(int)MathRound(bg+(rg-bg)*mix);
   int b=(int)MathRound(bb+(rb-bb)*mix);
   return (color)((b<<16)|(g<<8)|r);
  }

void StoreChannelOriginalView(const string chan)
  {
   if(ObjectFind(0,chan)<0)
      return;
   string gv_color=ChannelViewGV(chan);
   string gv_tf=ChannelViewTFGV(chan);
   if(!GlobalVariableCheck(gv_color))
      GlobalVariableSet(gv_color,(double)(long)ObjectGetInteger(0,chan,OBJPROP_COLOR));
   if(!GlobalVariableCheck(gv_tf))
      GlobalVariableSet(gv_tf,(double)(long)ObjectGetInteger(0,chan,OBJPROP_TIMEFRAMES));
  }

color ChannelOriginalColor(const string chan)
  {
   string gv=ChannelViewGV(chan);
   if(GlobalVariableCheck(gv))
      return (color)(long)GlobalVariableGet(gv);
   if(ObjectFind(0,chan)>=0)
      return (color)ObjectGetInteger(0,chan,OBJPROP_COLOR);
   return clrDodgerBlue;
  }

long ChannelOriginalTimeframes(const string chan)
  {
   string gv=ChannelViewTFGV(chan);
   if(GlobalVariableCheck(gv))
      return (long)GlobalVariableGet(gv);
   if(ObjectFind(0,chan)>=0)
      return ObjectGetInteger(0,chan,OBJPROP_TIMEFRAMES);
   return OBJ_ALL_PERIODS;
  }

void RestoreChannelView(const string chan,const bool forget=false)
  {
   if(ObjectFind(0,chan)>=0)
     {
      ObjectSetInteger(0,chan,OBJPROP_COLOR,ChannelOriginalColor(chan));
      ObjectSetInteger(0,chan,OBJPROP_TIMEFRAMES,ChannelOriginalTimeframes(chan));
     }
   if(forget)
     {
      GlobalVariableDel(ChannelViewGV(chan));
      GlobalVariableDel(ChannelViewTFGV(chan));
     }
  }

void ApplyActiveChannelView(const string chan)
  {
   if(ObjectFind(0,chan)<0)
      return;

   StoreChannelOriginalView(chan);
   color orig=ChannelOriginalColor(chan);
   long orig_tf=ChannelOriginalTimeframes(chan);
   color wanted=orig;
   long wanted_tf=orig_tf;

   if(InpActiveChannelView==CHANNEL_VIEW_PALE)
     {
      double mix=InpActiveChannelPaleMix;
      if(mix<0.10) mix=0.10;
      if(mix>0.90) mix=0.90;
      color back=(color)ChartGetInteger(0,CHART_COLOR_BACKGROUND);
      wanted=BlendColor(orig,back,mix);
     }
   else if(InpActiveChannelView==CHANNEL_VIEW_HIDE)
     {
      // OBJPROP_HIDDEN скрывает объект только в списке объектов.
      // OBJPROP_TIMEFRAMES=0 действительно убирает его с графика.
      wanted_tf=0;
     }

   bool changed=false;
   if((color)ObjectGetInteger(0,chan,OBJPROP_COLOR)!=wanted)
     {
      ObjectSetInteger(0,chan,OBJPROP_COLOR,wanted);
      changed=true;
     }
   if(ObjectGetInteger(0,chan,OBJPROP_TIMEFRAMES)!=wanted_tf)
     {
      ObjectSetInteger(0,chan,OBJPROP_TIMEFRAMES,wanted_tf);
      changed=true;
     }
   if(changed)
      ChartRedraw();
...
void RestoreAllChannelViews(const bool forget=false)
  {
   int n=StringLen(DEQ_VIEW_GV_PREFIX);
   for(int k=GlobalVariablesTotal()-1;k>=0;k--)
     {
      string gv=GlobalVariableName(k);
      if(StringFind(gv,DEQ_VIEW_GV_PREFIX)!=0)
         continue;
      string chan=StringSubstr(gv,n);
      RestoreChannelView(chan,forget);
     }
  }
...
color TraceColorB(const int i)
  {
   return ChannelOriginalColor(g_ch[i].name);
  }

color TraceColorD(const int i)
  {
   long base=ChannelOriginalColor(g_ch[i].name);
   if(base==0)
      base=clrDodgerBlue;
      ...
      if(ObjectFind(0,g_ch[i].name)<0)
        {
         ...
         GlobalVariableDel(ChannelViewGV(g_ch[i].name));
         ChRemove(i);
        }
   ...
   for(int k=0;k<total;k++)
     {
      ...
      if(StringFind(desc,"DEQ")==0)
        ...
        {
         RestoreChannelView(g_ch[i].name,true);
...
void ActivateChannel(const int i)
  {
   g_ch[i].state=CS_OFF;
   RestoreChannelView(g_ch[i].name,false);
   if(!ReadAnchors(i))
      return;
   if(!GeometryOK(i))
     {
   ...
   ApplyActiveChannelView(g_ch[i].name);
  }
      ...
      if(g_ch[i].state==CS_ACTIVE)
        {
         ...
         OnTickCheck(i);
         // Свойство объекта может быть переписано терминалом/шаблоном.
         // Пока динамика активна, поддерживаем выбранный вид.
         ApplyActiveChannelView(g_ch[i].name);
        }
      UpdateLabels(i);
     }
