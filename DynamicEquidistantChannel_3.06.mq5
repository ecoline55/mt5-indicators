...
void StoreChannelOriginalColor(const string chan)
  {
   if(ObjectFind(0,chan)<0)
      return;
   string gv=ChannelViewGV(chan);
   if(!GlobalVariableCheck(gv))
      GlobalVariableSet(gv,(double)(long)ObjectGetInteger(0,chan,OBJPROP_COLOR));
  }
...
void ApplyActiveChannelView(const string chan)
  {
   if(ObjectFind(0,chan)<0)
      return;
   StoreChannelOriginalColor(chan);
   color orig=ChannelOriginalColor(chan);
   if(InpActiveChannelView==CHANNEL_VIEW_AS_IS)
     {
      ObjectSetInteger(0,chan,OBJPROP_COLOR,orig);
      return;
     }
   if(InpActiveChannelView==CHANNEL_VIEW_HIDE)
     {
      ObjectSetInteger(0,chan,OBJPROP_COLOR,clrNONE);
      return;
     }
   ...
   ObjectSetInteger(0,chan,OBJPROP_COLOR,BlendColor(orig,back,mix));
  }
